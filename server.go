...
func MakeServer(serverId int, peerIds []int) *Server {
	s := new(Server)
	s.serverId = serverId
	s.peerIds = peerIds
...
func (rpp *RPCProxy) RequestVote(args Ballot, reply *VoteReply) error {
...
func (rpp *RPCProxy) AppendEntries(args EntryArgs, reply *EntryReply) error {
	if len(os.Getenv("RAFT_UNRELIABLE_RPC")) > 0 {
		dice := rand.Intn(10)
		if dice == 9 {
			return fmt.Errorf("RPC failed")
